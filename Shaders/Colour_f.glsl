...
vec3 kernel(in vec2 tex, in sampler2D image, in mat3 kernel)
{
	...
	output += kernel[0].x * texture(image, tex + (vec2(-1, 1) * size)).rgb;
	output += kernel[0].y * texture(image, tex + (vec2(-1, 0) * size)).rgb;
	output += kernel[0].z * texture(image, tex + (vec2(-1,-1) * size)).rgb;
												
	output += kernel[1].x * texture(image, tex + (vec2( 0, 1) * size)).rgb;
	output += kernel[1].y * texture(image, tex + (vec2( 0, 0) * size)).rgb;
	output += kernel[1].z * texture(image, tex + (vec2( 0,-1) * size)).rgb;
														
	output += kernel[2].x * texture(image, tex + (vec2( 1, 1) * size)).rgb;
	output += kernel[2].y * texture(image, tex + (vec2( 1, 0) * size)).rgb;
	output += kernel[2].z * texture(image, tex + (vec2( 1,-1) * size)).rgb;
